...
def train_net(net, X_train, y_train, n_epochs=500, lr=1e-3, device=None):
    if device is None:
...
def train_net_with_batches(net, X_train, y_train, n_epochs=500, lr=1e-3, batch_size=32, device=None):
...
def baseline_bagging(X_eval, y_eval, X_bagging_source, y_bagging_source, num_classes, n_models=5, n_epochs=500, device=None):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for _ in range(n_models):
        ...
        net_model = train_net(net_model, X_base_train, y_base_train, n_epochs=n_epochs, device=device)
        net_model.eval()
        with torch.no_grad():
...
def simple_ensemble_nn(X_eval, y_eval, X_training_data, y_training_data, num_classes,
                       n_models=5, n_epochs=500, device=None):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    n_samples_train, d_train = X_training_data.shape
    if n_samples_train == 0 or d_train == 0:
    ...
    for _ in range(n_models):
        net_model = Net(d_train, num_classes).to(device)
        net_model = train_net(net_model, X_training_data, y_training_data, n_epochs=n_epochs, device=device)
        net_model.eval()
        with torch.no_grad():
            X_tensor_eval = torch.tensor(X_eval, dtype=torch.float32).to(device)
            logits_eval = net_model(X_tensor_eval)
...
def calculate_stability_and_accuracy(method, X_eval, y_eval, X_train_source, y_train_source,
    ...
    if n_train_samples_source == 0 or input_dim_processed == 0:
        ...
        return np.nan, np.nan, [], np.nan, np.nan  # Added std returns as NaN
        ...
        try:
            ...
            if method == 'standard':
                current_pred_on_X_eval, current_acc_on_X_eval = baseline_bagging(
                    ...
                    n_models=n_models_arg, n_epochs=n_epochs_arg, device=device
                )
...
def main():
    ...
    # --- Data Loading for make_moons ---
    print("Generating make_moons dataset...")
    n_samples_moons = 2000
    noise_moons = 0.15
    X_moons, y_moons = make_moons(n_samples=n_samples_moons, noise=noise_moons, random_state=42)
    scaler = StandardScaler()
    X_scaled_moons = scaler.fit_transform(X_moons)

    # Split into train and test sets
    X_train, X_test, y_train, y_test = train_test_split(
        X_scaled_moons, y_moons, test_size=0.5, random_state=42, stratify=y_moons
    )

    print(f"Train set: {X_train.shape[0]} samples, Test set: {X_test.shape[0]} samples")

    num_classes = len(np.unique(y_moons))
    input_dim_processed = X_train.shape[1]  # Use train shape
    dataset_id_name = "make_moons"

    n_models_ensemble_list = list(range(2,8)) #[5, 7, 9, 11, 13, 15, 17, 20]
    n_bootstrap_stability_runs = 100
    n_epochs_training = 500
    common_glue_weight = 0.2
    hybrid_glue_subsample_ratio = 0.5  # Add this near your other config variables if not present
    ...
    # When calling calculate_stability_and_accuracy, capture the std values
    s_single, a_single, d_list_single, s_single_std, a_single_std = calculate_stability_and_accuracy(
        ...
        n_epochs=n_epochs_training, bootstrap_seed=outer_bootstrap_seed, n_models=1
    )
    # Store the std values
    ...
    for n_k_models in n_models_ensemble_list:
        ...
        for method_name, params in methods_to_run:
            ...
            s, a, d_list, s_std, a_std = calculate_stability_and_accuracy(
                ...
                bootstrap_seed=outer_bootstrap_seed, **params
            )
        ...
        # --- ADD THE STATISTICAL TEST BLOCK HERE ---
    ...
    # --- Pairwise statistical tests for stability means for this K ---
    from scipy.stats import mannwhitneyu
    ...
    # Save accuracies_by_dataset and stabilities_by_dataset as JSON
    def convert_np(obj):
    ...
    with open("accuracies_by_dataset_moons.json", "w") as f:
        json.dump(accuracies_by_dataset, f, indent=2, default=convert_np)
    with open("stabilities_by_dataset_moons.json", "w") as f:
